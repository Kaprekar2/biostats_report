...
##年度(と誤差頂+切片)だけを含んだモデル
model4 <- lm(yield ~ site + year, data = barley)
...
AICに基づくと以下のmodel7が最も評価が良く、次点で最初に提案したモデルが良いという結果が得られた.
...
という解釈になった.
このことからモデルとしては「栽培地」と「栽培地と年度の相互作用」を効果量として品種は処理効果には導入しないものが適当であると考えられる.

AICを元に選択された最適なモデルは、「栽培地」と「年度」と「栽培地と年度の相互作用」を効果量として、切片のランダム効果として品種を導入したものだった. 次点のモデルは効果量として「栽培地」と「品種」に加えて「栽培地と年度の相互作用」を導入したものだった.
...
また年度を独立した変数としてモデルに導入するよりも、交互作用項として組み込むとモデルの評価が良いことも以下のふたつのモデルの結果から分かる.
...
これらのことから、**栽培地を主な変量効果として設定**し、栽培地に**年度との交互作用を加え**、品種を処理項ではなく**ランダム効果として加える**という方針でモデルを立てることでこのデータを説明できる確からしいモデルが表現できると考えられる.
